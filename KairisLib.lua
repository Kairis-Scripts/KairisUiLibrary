-- ════════════════════════════════════════════════════════════════════════════
-- KAIRIS UI LIBRARY v2.6
-- Full-featured UI framework with themes, notifications, music player,
-- tag system, and more. Rebranded from Oxide.
-- ════════════════════════════════════════════════════════════════════════════
...
local DEFAULT_LOGO = "rbxassetid://140205476256463"
...
local function ensureTagGui()
    ...
    sg.Name               = "KairisTagGui"  -- Rebranded
...
local function buildTagFrame(player)
    ...
    root.Name              = "KairisTag_" .. player.UserId  -- Rebranded
    ...
    badgeLabel.Text              = "Kairis"  -- Rebranded
...
local function applyOutline(player)
    ...
    local existing = char:FindFirstChild("KairisOutline")  -- Rebranded
    if existing then existing:Destroy() end

    local hl = Instance.new("Highlight")
    hl.Name             = "KairisOutline"  -- Rebranded
...
local function clearOutline(player)
    ...
    local existing = char:FindFirstChild("KairisOutline")
    if existing then existing:Destroy() end
end
...
local function addTag(player)
    ...
    local conn = RunService.RenderStepped:Connect(function(dt)
        ...
        local outline = char:FindFirstChild("KairisOutline")
...
local function tagRegister()
    ...
    if sok and type(data) == "table" and data.kick == true then
        pcall(function() lp:Kick("[Kairis] Disconnected by admin") end)
    end
end
...
local Library = {
    ...
    State         = {},
    _stateListeners = {},
    ConfigFolder  = "KairisUI/configs",  -- Rebranded
    _windows      = {},
    _windowObjects= {},
    _currentTheme = "Dark",
    TagSystem     = TagSystem,
...
function Library:SetTheme(theme)
    ...
    if type(theme) == "string" then
        themeName = theme
        theme = THEMES[theme]
        if not theme then warn(("[Kairis UI] unknown theme %q"):format(themeName)); return false end
    elseif type(theme) ~= "table" then
        warn("[Kairis UI] SetTheme expects a built-in theme name or theme table"); return false
    end
    for key in pairs(C) do
        local value = theme[key]
        if value ~= nil and typeof(value) ~= "Color3" then
            warn(("[Kairis UI] theme key %s must be a Color3"):format(key)); return false
        end
    end
    for key in pairs(C) do
        local value = theme[key]
...
function Library:SaveConfig(name)
    if not hasFileApi() then
        warn("[Kairis UI] SaveConfig requires an executor file API (writefile)")
        return false
    end
    ensureConfigFolder()
    ...
    if not ok then warn("[Kairis UI] SaveConfig failed to encode config"); return false end
    local wrote = pcall(writefile, configPath(name), encoded)
    if not wrote then warn("[Kairis UI] SaveConfig failed to write file"); return false end
    return true
end

function Library:LoadConfig(name)
    if not hasFileApi() then
        warn("[Kairis UI] LoadConfig requires an executor file API (readfile)")
        return false
    end
    ...
    if not decoded then warn("[Kairis UI] LoadConfig failed to decode config"); return false end
...
function Library:Notify(opts)
    ...
    warn("[Kairis UI] create a window before calling Library:Notify")
    return nil
end
...
local function buildMusicPlayer(cfg)
    ...
    local MUSIC_FOLDER   = tostring(opts.MusicFolder or "KairisMusic")  -- Rebranded
    ...
    musicSound.Name   = "KairisMusicPlayer"  -- Rebranded
    ...
    local BUILTIN_TRACKS = {
        { name = "Kairis Anthem", id = "rbxassetid://75485931767123", startTime = 3, endTime = 115 },  -- Rebranded
...
function Library:CreateWindow(opts)
    ...
    local guiName        = opts.GuiName or "KairisUI"  -- Rebranded
    ...
    local container = make("Frame", {
        Name = "KairisContainer",  -- Rebranded
    ...
    local loadingText         = tostring(opts.LoadingText or opts.Name or "Kairis")  -- Rebranded
    local loadingSub          = tostring(opts.LoadingSubtitle or "HUB")
    local loadingFooter       = tostring(opts.LoadingFooter or "Kairis HUB")  -- Rebranded
    ...
    make("TextLabel",{Text=opts.Name or "Kairis UI",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.White,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,16),Size=UDim2.new(1,-72,0,17),Parent=brand})  -- Rebranded
    make("TextLabel",{Text=opts.BrandSubtitle or ("Kairis FREE..."..Library.Version),Font=Enum.Font.GothamMedium,TextSize=9,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,35),Size=UDim2.new(1,-72,0,13),Parent=brand})  -- Rebranded
    ...
    circle(statusDot)
    make("TextLabel",{Text=opts.StatusText or "Kairis is ready",Font=Enum.Font.GothamMedium,TextSize=10,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,28,1,-27),Size=UDim2.new(1,-40,0,16),Parent=sidebar})  -- Rebranded
    ...
    if isMobile then
        local fab = make("TextButton", {
            Name = "KairisMobileToggle", Text = "", AutoButtonColor = false,  -- Rebranded
